...
for hs in mhs:
    for DM in mDM:
        for Zp in mZp:
            print(signalDirectory+'/nanoLatino_DarkHiggs_MonoHs_HsToWWTo2l2nu_mhs_' + hs + '_mx_' + DM  + '_mZp_' + Zp)
            samples['DH_mhs_' + hs + '_mx_' + DM  + '_mZp_' + Zp] = {
                'name': nanoGetSampleDMFiles(signalDirectory, 'DarkHiggs_MonoHs_HsToWWTo2l2nu_mhs_' + hs + '_mx_' + DM  + '_mZp_' + Zp),
                'weight': mcCommonWeight,
                'FilesPerJob': 1
            }
    ...
    for hs in mhs:
        ...
        for Zp in mZp:
            ...
            if DM == '200' and hs == '400' and int(Zp) > 1000:
                continue
            print(signalDirectory+'/DarkHiggs_MonoHs_HsToWWTo2l2nu_mhs_' + hs + '_mx_' + DM  + '_mZp_' + Zp)
            samples['DH_mhs_' + hs + '_mx_' + DM  + '_mZp_' + Zp] = {
                'name': nanoGetSampleDMFiles(signalDirectory, 'DarkHiggs_MonoHs_HsToWWTo2l2nu_mhs_' + hs + '_mx_' + DM  + '_mZp_' + Zp), 
                'weight': mcCommonWeight,
                'FilesPerJob': 1
